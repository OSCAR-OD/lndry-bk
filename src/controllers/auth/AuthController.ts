...
class AuthController {
  async register(req: Request, res: Response) {
    try {
      const {name, email, phone, password, referCode} = req.body;
      ...
      const created = await userRepo.add({name, email, phone, password, referCode});
      if (created)
        return res
          .status(OK)
  ...
  async login(req: Request, res: Response) {
    try {
      ...
      const token = jwt.sign({email: user.email?.toString(), name: user.name, role: user.role, image: user.image}, envVars.jwt.secret, {
      ...
      return res
        ...
        .send(
          success(
            ...
            {
              user: {...data, previousOrderPostCode: user.previousOrderPostCode, referralCode: user.referralCode},
              accessToken: token,
  ...
  async resetPasswordEmail(req: Request, res: Response) {
    try {
      ...
      const link = `https://beta.nearestlaundry.com/reset/${data}`;
