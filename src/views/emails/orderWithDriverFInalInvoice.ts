export default (data: any) => {
    ...
    data.items.map((item: any, index: number) => {
        trs += `<tr style="background-color: rgba(255,255,255,0.2);color: #100f0f;">
                ...
                <td style="height: 40px; border-bottom: 1px solid #e7e0e0;">£ ${(item.quantity * item.price).toFixed(2)}</td>
            </tr>`;
    });
    let updated: string = '';
        ...
        <td>
            <div>
                ...
                <div style="width: 100%;clear: both;padding-top: 40px;">
                    <strong>Dear ${data.name},</strong> <br> You ordered bellow item(s) with our order with driver option.
                </div>
                <!-- table -->
                <table style="width: 100%;
                    border-collapse: collapse;
